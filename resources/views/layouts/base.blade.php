...
<body>
   ...
   @if(Route::has('login'))
       ...
       @elseif(Auth::user()->utype==='SVP')
                 <li class="login-form"> <a href="{{route('register')}}" title="Register">MY Account (Service Provider)</a>
                  <ul class="drop-down one-column hover-fade">
                            <li><a href="{{route('ServiceProvider_pro.show', Auth::user()->name )}}">My Acount (Profile) {{Auth::user()->name}} </a></li>
                            <li><a href="{{route('logout')}}" onclick="event.preventDefault(); document.getElementById('logout-form').submit();">Logout</a></li>
                        </ul>
                </li>

                @else
                 <li class="login-form"> <a href="{{route('register')}}" title="Register">MY Account (Custumer)</a> {{Auth::user()->name}}
                  <ul class="drop-down one-column hover-fade">
...
@yield('content')
  ...
  <script type="text/javascript">
    ...
    <!-- when select usertype(customer) the Experiance/category/sub category will be hidden -->
    ...
    <script>
        function changestatus()
        {
            ...
            if(status.value == "CST")
            {
                ...
                document.getElementById("exp_hide_if_cst_select").style.visibility= "visibile";
            }
...
<script>
        $(document).ready(function () {
            $('select[name="category"]').on('change', function () {
                ...
                if (category) {
                    $.ajax({
                        url: "{{ URL::to('Get_subcategory') }}/" + category,
                        type: "GET",
                        dataType: "json",
                        success: function (data) {
                            $('select[name="subcategory"]').empty();
                            $('select[name="subcategory"]').append('<option selected disabled >Choose...</option>');
